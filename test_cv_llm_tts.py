...
async def face_date_create(websocket_tts):
    # 顔認識モデルの読み込み
    ...
    while(True):
        ...
        if faces is not None and not (len(faces)==0) and not is_generating:
...
async def run_test():
    ...
    async with websockets.connect('ws://localhost:8766') as websocket_tts:
        # while True:
        try:
            await face_date_create(websocket_tts)
        except websockets.exceptions.ConnectionClosedError as e:
            print(f"Retrying LLM connection due to error: {e}")
            websocket_tts = await connect_with_retry('ws://localhost:8766')
            await face_date_create(websocket_tts)
